...
public class GitHubHttpServer implements com.linkall.vance.core.http.HttpServer {
    ...
    public GitHubHttpServer(){
        ...
        this.handlerRI = new HttpResponseInfo(200, "Receive success, deliver CloudEvents to" + EnvUtil.getVanceSink() + "success", 500, "Receive success, deliver CloudEvents to" + EnvUtil.getVanceSink() + "failure");
    }
    ...
    public void listen() {
        ...
        this.httpServer.listen(port, (server) -> {
            if (server.succeeded()) {
                LOGGER.info("HttpServer is listening on port: " + ((io.vertx.core.http.HttpServer)server.result()).actualPort());
            } else {
    ...
    public static void main(String[] args) {
        ...
        server.init();
        server.simpleHandler(new GitHubHttpAdapter());
        server.listen();
    }
}
